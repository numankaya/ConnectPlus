        ...
        children: [
          ...
          ElevatedButton(onPressed: () async{
            if (password.text == passwordAgain.text) {
              final bool isSuccess = await FirebaseAuthServiceMethods(FirebaseAuth.instance).signUpWithEmail(fullName: fullName.text, email: mail.text, password: password.text, context: context);
              if (isSuccess) {
                CreateProviderDatas();
              }
            }
          ...
          ElevatedButton(onPressed: () async{
            if (fullName.text.isNotEmpty) {
              final bool isSuccess = await FirebaseAuthServiceMethods(FirebaseAuth.instance).SignInWithGoogle(context, fullName: fullName.text, isCreatingAcc: true);
              if (isSuccess) {
                CreateProviderDatas();
              }
            }
